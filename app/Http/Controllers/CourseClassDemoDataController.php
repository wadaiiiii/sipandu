...
class CourseClassDemoDataController extends Controller
{
    public function __invoke(
        ...
        CourseClassMeetingService $meetingService,
    ): JsonResponse {
        abort_unless($this->canEdit($request->user(), $courseClass), 403);

        $meetingService->ensureDefaultSlots($courseClass);
        ...
        return response()->json([
            ...
            'message' => $createdMaterials + $createdAssignments > 0
                ? "Data contoh ditambahkan: {$createdMaterials} materi dan {$createdAssignments} tugas."
