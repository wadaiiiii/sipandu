...
function StudentClassroom() {
    ...
    const load = async () => {
        ...
        const [bootstrapRes, roomRes, announcementRes, dashboardRes] = await Promise.all([
            api('/api/bootstrap'), api(`/api/classes/${classId}/meetings`),
            api(`/api/classes/${classId}/announcements`), api('/api/dashboard'),
        ]);
    ...
    const toggleMaterial = async (material: Material) => {
        ...
        const response = await api(`/api/classes/${room.class.id}/materials/${material.id}/learned`, { method: 'PUT', body: JSON.stringify({ learned: !material.is_learned }) });
    ...
    const upload = async (file: File): Promise<string | null> => {
        ...
        const response = await api(`/api/classes/${room.class.id}/files`, { method: 'POST', body: form });
        if (!response.ok) {
    ...
    const submit = async (event: FormEvent, assignment: Assignment) => {
        ...
        const response = await api(`/api/classes/${room.class.id}/assignments/${assignment.id}/submission`, { method: 'POST', body: JSON.stringify({ answer_text: draft.answer_text || null, attachment_url: attachmentUrl || null }) });
        if (!response.ok) setError(await responseError(response));
        else {
