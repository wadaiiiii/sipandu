...
function App() {
    ...
    const loadList = async () => {
        ...
        const [bootstrap, list] = await Promise.all([api('/api/bootstrap'), api(`/api/classes/${classId}/quizzes`)]);
    ...
    const loadDetail = async (id: number) => {
        setBusy(true); setError('');
        const response = await api(`/api/classes/${classId}/quizzes/${id}`);
    ...
    const createQuiz = async (event: FormEvent) => {
        ...
        const response = await api(`/api/classes/${classId}/quizzes`, { method: 'POST', body: JSON.stringify(quizPayload(quizForm)) });
        if (!response.ok) { setError(await err(response)); setBusy(false); return; }
    ...
    const saveQuiz = async () => {
        ...
        const response = await api(`/api/classes/${classId}/quizzes/${detail.id}`, { method: 'PATCH', body: JSON.stringify(quizPayload(quizForm)) });
    ...
    const addQuestion = async (event: FormEvent) => {
        ...
        const response = await api(`/api/classes/${classId}/quizzes/${detail.id}/questions`, { method: 'POST', body: JSON.stringify(payload) });
        if (!response.ok) setError(await err(response));
    ...
    const deleteQuestion = async (questionId: number) => {
        ...
        const response = await api(`/api/classes/${classId}/quizzes/${detail.id}/questions/${questionId}`, { method: 'DELETE' });
        if (!response.ok) setError(await err(response)); else { setNotice('Soal dihapus.'); await loadDetail(detail.id); }
    };

    const startQuiz = async () => {
        if (!detail) return; setBusy(true); setError('');
        const response = await api(`/api/classes/${classId}/quizzes/${detail.id}/start`, { method: 'POST' });
        if (!response.ok) setError(await err(response));
        else {
    ...
    const saveAnswer = async (question: Question, value: Record<string, unknown>) => {
        ...
        const response = await api(`/api/classes/${classId}/quizzes/${detail.id}/attempts/${attempt.id}/questions/${question.id}`, { method: 'PUT', body: JSON.stringify({ answer: value }) });
        if (!response.ok) setError(await err(response));
    };
    ...
    const submitQuiz = async () => {
        ...
        setBusy(true); setError('');
        const response = await api(`/api/classes/${classId}/quizzes/${detail.id}/attempts/${attempt.id}/submit`, { method: 'POST' });
        if (!response.ok) setError(await err(response));
    ...
    const gradeEssay = async (attemptId: number, answer: Answer, score: number, feedback: string) => {
        ...
        setBusy(true); setError('');
        const response = await api(`/api/classes/${classId}/quizzes/${detail.id}/attempts/${attemptId}/answers/${answer.id}/grade`, { method: 'PATCH', body: JSON.stringify({ score, feedback: feedback || null }) });
