...
function buildRequestCard(member: JoinMember): HTMLElement {
    ...
    const process = async (decision: 'approve' | 'reject') => {
        ...
        const response = await api(`/api/classes/${classId}/join-requests/${member.id}/${decision}`, { method: 'PATCH' });
        if (!response.ok) {
...
async function load(): Promise<void> {
    ...
    loading = true;
    try {
        const bootstrapResponse = await api('/api/bootstrap');
        ...
        const classesResponse = await api('/api/classes');
