...
function Classroom() {
    ...
    const load = async () => {
        ...
        const [classesResponse, announcementsResponse] = await Promise.all([
            api('/api/classes'),
            api(`/api/classes/${classId}/announcements`),
        ]);
    ...
    const uploadFile = async (file: File, purpose: 'material' | 'assignment' | 'submission'): Promise<UploadedFileResult | null> => {
        ...
        const response = await api(`/api/classes/${classId}/files`, {
            method: 'POST',
            body: form,
        });
    ...
    const deleteAnnouncement = async (announcement: Announcement) => {
        ...
        await mutate(
            () => api(`/api/classes/${classId}/announcements/${announcement.id}`, { method: 'DELETE' }),
            'Pengumuman dihapus.',
        );
    };
    ...
    const addMaterial = async (event: FormEvent) => {
        ...
        const ok = await mutate(
            () => api(`/api/classes/${classId}/meetings/${targetMeeting.id}/materials`, {
                method: 'POST',
                body: JSON.stringify({
    ...
    const removeMaterial = async (meetingId: number, material: Material) => {
        if (!payload?.can_edit) return;
        await mutate(
            () => api(`/api/classes/${classId}/meetings/${meetingId}/materials/${material.id}`, { method: 'DELETE' }),
            'Materi dihapus.',
        );
    };
    ...
    const addAssignment = async (event: FormEvent) => {
        ...
        const ok = await mutate(
            () => api(`/api/classes/${classId}/meetings/${targetMeeting.id}/assignments`, {
                method: 'POST',
                body: JSON.stringify({
    ...
    const addParticipant = async (event: FormEvent) => {
        ...
        const ok = await mutate(
            () => api(`/api/classes/${classId}/participants`, {
    ...
    const removeParticipant = async (member: Member) => {
        ...
        await mutate(
            () => api(`/api/classes/${classId}/participants/${member.user.id}`, { method: 'DELETE' }),
