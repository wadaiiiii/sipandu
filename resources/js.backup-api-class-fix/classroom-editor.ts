...
function openMaterialEditor(material: Material): void {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        const response = await fetch(`/api/classes/${classId}/meetings/${material.meeting_id}/materials/${material.id}`, {
...
async function load(): Promise<void> {
    ...
    const [roomResponse, materialResponse] = await Promise.all([
        fetch(`/api/classes/${classId}/meetings`, { credentials: 'include', headers: { Accept: 'application/json' } }),
        fetch(`/api/classes/${classId}/material-resources`, { credentials: 'include', headers: { Accept: 'application/json' } }),
    ]);
