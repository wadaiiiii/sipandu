...
function LecturerQuiz({ quiz, form, setForm, questionForm, setQuestionForm, busy, savingQuestion, onSaveQuiz, onDeleteQuiz, onAddQuestion, onDeleteQuestion, onGradeEssay }: {
    ...
    return <div className="space-y-5">
        <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.15em] text-blue-600">Pengaturan kuis</p><h2 className="mt-1 text-2xl font-extrabold">{quiz.title}</h2><p className="mt-2 text-sm text-slate-500">Kunci jawaban hanya disimpan di server dan tidak dikirim ke akun mahasiswa.</p></div><button onClick={onSaveQuiz} disabled={busy} className="inline-flex items-center gap-2 rounded-2xl bg-blue-600 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60"><Save size={16}/>{busy ? 'Menyimpan...' : 'Simpan pengaturan'}</button><button type="button" title="Hapus kuis" aria-label="Hapus kuis" data-sipandu-native-quiz-delete="true" onClick={onDeleteQuiz} className="grid h-11 w-11 place-items-center rounded-2xl bg-rose-50 text-rose-600 transition hover:bg-rose-100"><Trash2 size={17} /></button></div>
