...
function StudentMaterialChecklist() {
    ...
    useEffect(() => {
        ...
        const run = async () => {
            ...
            const room = await loadJson<ClassroomPayload>(`/sipandu-api/classes/${classId}/meetings`);
    ...
    const toggle = async (material: Material) => {
        ...
        try {
            const response = await fetch(`/sipandu-api/classes/${payload.class.id}/materials/${material.id}/learned`, {
                method: 'PUT',
                credentials: 'include',
                headers: {
