...
function Classroom() {
    ...
    const [assignmentForm, setAssignmentForm] = useState<AssignmentForm>({
        title: '', instructions: '', max_score: '100', due_at: '', status: 'published',
    });
    ...
    const lecturers = useMemo(
        () => members.filter((member) => member.status === 'active' && member.membership_role === 'lecturer'),
        [members],
    );

    const students = useMemo(
        () => members.filter((member) => member.status === 'active' && member.membership_role === 'student'),
        [members],
    );
    ...
    if (!payload && busy) {
        return <div className="grid min-h-screen place-items-center bg-[#f4f7ff] text-sm font-semibold text-slate-500">Memuat ruang kelasï¿½</div>;
    }
                        ...
                        {draft && (
                            <section className="rounded-3xl border border-blue-100 bg-white p-5 shadow-sm sm:p-6">
                                ...
                                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                                    ...
                                    <Field label="Metode"><input disabled={!payload.can_edit} value={draft.learning_method ?? ''} onChange={(e) => setDraft({ ...draft, learning_method: e.target.value })} className="field" placeholder="Ceramah, diskusi, praktikumï¿½" /></Field>
                                </div>
                    ...
                    <div className="mt-5 grid gap-5 xl:grid-cols-[370px_minmax(0,1fr)]">
                        {payload.can_edit && (
                            <form onSubmit={addMaterial} className="self-start rounded-3xl border border-blue-100 bg-white p-5 shadow-sm xl:sticky xl:top-24">
                                ...
                                <Field label="URL"><input value={materialForm.resource_url} onChange={(e) => setMaterialForm({ ...materialForm, resource_url: e.target.value })} className="field" placeholder="https://ï¿½" /></Field>
                                ...
                                <button disabled={busy} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50"><FileUp size={15} /> {busy ? 'Memprosesï¿½' : 'Publikasikan materi'}</button>
                            </form>
                        )}
                                    ...
                                    <article key={assignment.id} className="rounded-3xl border border-blue-100 bg-white p-5 shadow-sm sm:p-6">
                                        ...
                                        {!isStudent && (
                                            <div className="mt-5 border-t border-slate-100 pt-4">
                                                <p className="text-sm font-bold">Pengumpulan mahasiswa <span className="text-slate-400">({assignment.submission_count})</span></p>
...
function HeroStat({ label, value }: { label: string; value: string }) {
    return <div className="min-w-[82px] rounded-2xl border border-white/15 bg-white/10 px-3 py-3 text-center backdrop-blur"><p className="text-xl font-bold">{value}</p><p className="mt-0.5 text-[10px] font-semibold uppercase tracking-wide text-blue-100">{label}</p></div>;
}
