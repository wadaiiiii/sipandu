...
function controlByLabel<T extends HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>(form: HTMLFormElement, labelText: string): T | null {
    const label = Array.from(form.querySelectorAll('label')).find((item) =>
        (item.childNodes[0]?.textContent ?? item.textContent ?? '').trim().toLowerCase().startsWith(labelText.toLowerCase()),
    );
...
function inlineMessage(form: HTMLFormElement, message: string, isError = false): void {
    ...
    if (!box) {
        ...
        box.className = 'mt-3 rounded-xl border px-3 py-2 text-xs font-semibold';
...
async function storeMaterialWithAttachment(form: HTMLFormElement, file: File): Promise<void> {
    ...
    if (!meeting?.value || !title?.value.trim()) {
        inlineMessage(form, 'Pertemuan dan judul materi wajib diisi.', true);
        return;
    }
    ...
    try {
        ...
        const response = await fetch(`/sipandu-api/classes/${classId}/meetings/${meeting.value}/materials`, {
            ...
            body: JSON.stringify({
                ...
                resource_type: resourceType?.value || 'document',
...
function enhanceMaterialForm(): void {
    ...
    if (fileLabel) {
        ...
        if (firstText?.textContent?.includes('Atau upload')) {
            firstText.textContent = 'Lampiran file (opsional, maks. 4 MB)';
        }
    }
    if (linkInput) linkInput.placeholder = 'https://… (boleh diisi bersamaan dengan file)';
...
async function fetchResources(): Promise<void> {
    ...
    try {
        const response = await fetch(`/sipandu-api/classes/${classId}/material-resources`, {
            credentials: 'include',
...
function findMaterialCard(resource: MaterialResource): HTMLElement | null {
    return Array.from(document.querySelectorAll<HTMLElement>('article')).find((article) => {
        ...
        if (title !== resource.title) return false;
        return (article.textContent ?? '').includes(`Pertemuan ${resource.meeting_number}`);
    }) ?? null;
}
...
function decorateMaterialCards(): void {
    resources.forEach((resource) => {
        ...
        anchor.dataset.materialAttachmentId = String(resource.id);
        ...
        const content = card.querySelector('h3')?.parentElement;
